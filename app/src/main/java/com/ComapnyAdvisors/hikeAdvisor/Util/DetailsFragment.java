...
import com.ComapnyAdvisors.hikeAdvisor.adapter.ViewPagerAdapter;
    ...
    public void onViewCreated(@NonNull @NotNull View view, @Nullable Bundle savedInstanceState) {
        ...
        parkViewModel.getSelectedPark().observe(this, park -> {
            ...
            for (int i = 0; i < park.getActivities().size(); i++) {
                stringBuilder.append(park.getActivities().get(i).getId())
                        .append(" | ");
            }
            activities.setText(stringBuilder);
            if (park.getEntranceFees().size() > 0) {
                entranceFees.setText("Cost: $%s" + park.getEntranceFees().get(0).getCost());
            } else {
